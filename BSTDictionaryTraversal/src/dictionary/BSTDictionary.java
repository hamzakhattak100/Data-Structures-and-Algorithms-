...
public class BSTDictionary {
    ...
    public ArrayList<WordNode> levelOrder() {
        // WRITE YOUR CODE HERE
        ArrayList <WordNode> bob = new ArrayList<>(); 
        Queue <WordNode> bob2 = new Queue<>();
        bob2.enqueue(root);
        while(bob2.isEmpty() == false)
        {
            WordNode bob3 = bob2.dequeue();
            bob.add(bob3);
            if(bob3.getLeft()!= null)
            {
                bob2.enqueue(bob3.getLeft());
            }
            if(bob3.getRight()!= null)
            {
                bob2.enqueue(bob3.getRight());
            }

        }
        return bob; // Replace this line, it's provided so the code compiles
    }
