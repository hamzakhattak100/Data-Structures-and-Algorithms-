 ...
 * It creates the main window, initializes the map panel, and handles user
 * interactions.
 *
 * @author Vian Miranda
 * @author Anna Lu
...
public class Driver {
    ...
    public void updateStreetInfo(Block block) {
        if (block == null) {
            ...
            blockTrafficFactorLabel
                    .setText(String.format("<html><b>Traffic Factor:</b> %.3f", block.getTrafficFactor()));
    ...
    public void updatePathInfo(Intersection start, Intersection end) {
        if (start == null && end == null) {
            ...
            dfsLabel.addMouseListener(new java.awt.event.MouseAdapter() {
                ...
                public void mouseClicked(java.awt.event.MouseEvent e) {
                    StringSelection stringSelection = new StringSelection(
                            formattedDfsPaths[1].replace("<br>", "").replace("<em>", "").replace("<em/>", ""));
            ...
            defaultBlockLengthValue = String.format(
                    ...
                    mapPanel.minimizeIntersectionPathColor.getRed(), mapPanel.minimizeIntersectionPathColor.getGreen(),
                    mapPanel.minimizeIntersectionPathColor.getBlue(), minPathInfo[0],
                    mapPanel.fastestPathColor.getRed(),
            ...
            defaultBlockTrafficFactorValue = String.format(
                    ...
                    mapPanel.minimizeIntersectionPathColor.getRed(), mapPanel.minimizeIntersectionPathColor.getGreen(),
                    mapPanel.minimizeIntersectionPathColor.getBlue(), minPathInfo[1],
                    mapPanel.fastestPathColor.getRed(),
            ...
            defaultBlockTrafficValue = String.format(
                    "<span style=\"color:rgb(%s,%s,%s);\">%.2f</span>, <span style=\"color:rgb(%s,%s,%s);\">%.2f</span>",
                    mapPanel.minimizeIntersectionPathColor.getRed(), mapPanel.minimizeIntersectionPathColor.getGreen(),
                    mapPanel.minimizeIntersectionPathColor.getBlue(), minPathInfo[2],
                    mapPanel.fastestPathColor.getRed(),
            ...
            minIntersectionsLabel.addMouseListener(new java.awt.event.MouseAdapter() {
                ...
                public void mouseClicked(java.awt.event.MouseEvent e) {
                    StringSelection stringSelection = new StringSelection(
                            formattedMinPaths[1].replace("<br>", "").replace("<em>", "").replace("<em/>", ""));
                    Clipboard clipboard = Toolkit.getDefaultToolkit().getSystemClipboard();
                    clipboard.setContents(stringSelection, null);
                }
            });
            ...
            fastestPathLabel.addMouseListener(new java.awt.event.MouseAdapter() {
                ...
                public void mouseClicked(java.awt.event.MouseEvent e) {
                    StringSelection stringSelection = new StringSelection(
                            formattedFastPaths[1].replace("<br>", "").replace("<em>", "").replace("<em/>", ""));
                    Clipboard clipboard = Toolkit.getDefaultToolkit().getSystemClipboard();
                    clipboard.setContents(stringSelection, null);
                }
            });
    ...
    private String[] formatDFSPath(ArrayList<Intersection> path) {
        ...
        sb.append("<br><em>total ").append(index)
                .append(index == 1 ? " intersection (click to copy)</em>" : " intersections (click to copy)</em>");
    ...
    private String[] formatPath(ArrayList<Intersection> path) {
        ...
        for (int i = 0; i < path.size() - 1; i++) {
            ...
            Block block = rumaps.getRutgers().adj(rumaps.getRutgers().findIntersection(current.getCoordinate().getX(),
